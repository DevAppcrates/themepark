...
class VideosController extends Controller
{
    ...
    public function create_image_session(Request $request)
    {
        ...
        if(apiHelper::authenticate($user_id,$auth_id)) {
            ...
            $video->video_id = $video_id;
            $video->user_id = $user_id;
            if($request->has('lost_person')){

                $video->lost_person = $request->lost_person;
            }
     ...
     public function sendMessageToContacts($user_id,$video_id)
    {
        ...
        try {
            Mail::send([], [], function ($message) use ($contacts_email,$type,$video_id) {
                if($type=='panic'){
                    $body = 'A new panic alert is in progress. View Here: https://controlhub.themeparkalerts.com/dashboard?type='.$type.'&video_id='.$video_id;
                }elseif($type=='lost_person'){
                    $body = 'A new lost person alert is in progress. View Here: https://controlhub.themeparkalerts.com/dashboard?type='.$type.'&video_id='.$video_id;
                }else{
                    $body = 'A new Report Tip has been submitted. View Here: https://controlhub.themeparkalerts.com/dashboard?type='.$type.'&video_id='.$video_id;
                }
                $message->to($contacts_email)->subject('ThemeParkAlert | Alert')->setBody($body, 'text/html');
            });
        }catch (\Exception $e)
        {
        ...
        if(apiHelper::authenticate($user_id,$auth_id)) {
            ...
            try {
                Mail::send([], [], function ($message) use ($user_contacts_email,$user_first_name) {
                    ...
                    $message->to($user_contacts_email)->subject('ThemeParkAlert | Your Friend Need Help')->setBody($body, 'text/html');
                });
            }catch (\Exception $e) {
    ...
    public function update_lost_person(Request $request){
        $user_id=$request->header('user-id');
        $auth_id=$request->header('auth-id');
        if(apiHelper::authenticate($user_id,$auth_id)) {
            $rules = array('video_id'=>'required','lost_person'=>'required');
            $validations = Validator::make($request->all(),$rules);
            if ($validations->fails())
            {
                $messages = $validations->messages();
